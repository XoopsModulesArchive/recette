...
function recette_search($queryarray, $andor, $limit, $offset, $userid){
	global $xoopsDB;
	$sql = "SELECT storyid,uid,title,created FROM ".$xoopsDB->prefix("recette").", ".$xoopsDB->prefix("recette_lnk_ingredient_recette")."  WHERE published>0 AND published<=".time()."";
	if ( $userid != 0 ) {
	...
	if ( is_array($queryarray) && $count = count($queryarray) ) {
		...
		$sql .= " OR ( storyid = recette AND ingredient LIKE '%$queryarray[0]%' ) )";
		for($i=1;$i<$count;$i++){
			$sql .= " $andor ";
	...
	$sql .= "GROUP BY storyid ORDER BY created DESC";
