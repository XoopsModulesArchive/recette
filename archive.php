...
$xoopsOption['template_main'] = 'recette_archive.html';
...
$xoopsTpl->assign('xoops_pagetitle', $myts->makeTboxData4Show($xoopsModule->name()) . ' - ' . $myts->makeTboxData4Show(_NW_NEWSARCHIVES) . $pgtitle);
...
$result = $xoopsDB->query("SELECT published FROM ".$xoopsDB->prefix("recette")." WHERE published>0 AND published<=".time()." AND expired <= ".time()." ORDER BY published DESC");
if (!$result) {
	exit();
...
if ($fromyear != 0 && $frommonth != 0) {
	...
	$sql = "SELECT * FROM ".$xoopsDB->prefix("recette")." WHERE published >= $monthstart and published <= $monthend ORDER by published DESC";
