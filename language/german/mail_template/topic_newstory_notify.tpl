Hallo {X_UNAME},

Ein neues Rezept "{STORY_NAME}" wurde auf {X_SITENAME} in der Kategorie: {TOPIC_NAME} veröffentlicht.

Sie finden die Einsendung unter folgender URL:
{STORY_URL}

-----------

Sie erhalten diese E-Mail/PN weil Sie benachrichtigt werden wollen wenn neue Rezepte eingeschickt werden.

Wenn dies ein Fehler ist und Sie keine weiteren Nachrichten dieser Art mehr empfangen wollen, folgen Sie diesem Link:
{X_UNSUBSCRIBE_URL}

Bitte nicht auf diese E-Mail/PN antworten.

-----------

{X_SITENAME} ({X_SITEURL}) 
Der Webmaster
{X_ADMINMAIL}
